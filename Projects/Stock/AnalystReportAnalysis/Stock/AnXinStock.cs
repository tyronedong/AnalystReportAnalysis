...
namespace Stock
{
    public class AnXinStock : StockData
    {
         List<string> subjects;
         public AnXinStock() { }
         public AnXinStock(StockData stockData) //高于人名的号码不可以要，抽出来的高于人名的号码可能是假的
        {
            ...
            subjects.Add("■");//■ 事件： 公司拟与北京世纪创展投资管理有限公司共同发起设立总规
        }
        ...
        //抽取pdf中的内容
         public string extractContent(string[] lines)
         {
             ...
             for (int i = 0; i < lines.Length; i++)
             {
                 ...
                 if (temp.Contains("本报告版权属于安信证券股份有限公司"))
                     continue;
                 if (temp.Contains("各项声明请参见报告尾页"))
                     continue;
                 if (temp.Contains("敬请参阅报告结尾处免责声明"))
                     continue;
                 if (temp.Contains("敬请阅读本报告正文后各项声明"))
                     continue;
                 if (temp.StartsWith("公司快报"))
                     continue;
                 if (temp.StartsWith("公司动态分析"))
                     continue;
                 if (temp.StartsWith("公司深度分析"))
                     continue;
