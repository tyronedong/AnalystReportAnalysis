...
{
    public class PingAnSecurities : ReportParser
    {
        //平安证券
        public PingAnSecurities(string pdReportPath)
            : base(pdReportPath)
        {
            if (this.isValid)
            {
        ...
        public override bool extractStockOtherInfo()
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (!hasPriceMatched && trimedLine.StartsWith("现价："))
                {
                    if (stockPrice.IsMatch(trimedLine))
                    {
                        ...
                        {
                            Trace.TraceError("PingAnSecurities.extractStockOtherInfo(): " + e.Message);
                        }
                    }
                }
            }
        ...
        public override string[] removeAnyButContentInParas(string[] paras)
        {
            ...
            Regex extra2 = new Regex(@"（\d{6}）\d{6}：");
            ...
            foreach (var para in paras)
            {
                ...
                if (trimedPara.Contains("请务必阅读正文之后的免责条款"))//added
                {
                    continue;
                }
