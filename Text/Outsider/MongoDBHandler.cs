...
namespace Text.Outsider
{
    ...
    class MongoDBHandler
    {
        ...
        public AnalystReport FindXthOne(int quidRank)
        {
            MongoCursor<AnalystReport> cursor = FormulateCursor(quidRank);
            List<AnalystReport> tempList = new List<AnalystReport>();

            if (cursor == null)
            {
                Trace.TraceWarning("Report.Handler.MongoDBHandler.FindXthOne goes wrong");
                return null;
            }
            ...
            {
                Trace.TraceWarning("Report.Handler.MongoDBHandler.FindXthOne: " + e.ToString());
                return null;
            }

            try
            {
                return cursor.ElementAt(0);
            }
            catch (Exception e)
            {
                Trace.TraceWarning("Report.Handler.MongoDBHandler.FindXthOne: " + e.ToString());
