...
{
    //申银万国的抽取方法
    public class ShenYinStock:StockData
    { 
         List<string> subjects;
         public ShenYinStock() { }
         public ShenYinStock(StockData stockData) //高于人名的号码不可以要，抽出来的高于人名的号码可能是假的
        {
            ...
            subjects.Add("投资案件");
            subjects.Add("关键假设点");
            subjects.Add("有别于大众的认识");
            subjects.Add("股价表现的催化剂");
            subjects.Add("证券分析师承诺");
            subjects.Add("与公司有关的信息披露");
            subjects.Add("股票投资评级说明");
            subjects.Add("法律声明");
        }
         ...
         public void addSubjectsFromMulu(string[] lines)
         {
             ...
             {
                 if (lines[i].Replace(" ", "").Equals("目录") || lines[i].Replace(" ", "").Equals("目录"))
                 {
                     i++;
                     break;
                 }
             ...
             for (i = 0; i < lines.Length; i++)
             {
                 if (lines[i].Replace(" ", "").Equals("正文目录"))
                 {
                     i++;
                     break;
                 }
             }
             string regexNum2 = "[一|二|三|四|五|六|七|八|九]";
             Regex regNum2 = new Regex(regexNum);
             for (; i < lines.Length; i++)
             {
                 if (regNum2.IsMatch(lines[i]))
                 {
                     Regex reg1 = new Regex("[\u4E00-\u9FA5]+");
                     var mat = reg1.Match(lines[i]);
                     subjects.Add(mat.ToString());
         ...
         public bool isSubject(string line)
         {
             for (int j = 0; j < subjects.Count; j++)
             {
                 ...
                 if (temp.EndsWith(subjects[j]) || temp.StartsWith(subjects[j]) || temp.EndsWith(subjects[j] + "："))
                 {
                     Regex reg1 = new Regex("[\u4E00-\u9FA5]+");
         ...
         public string extractContent(string[] lines)
         {
             ...
             for (int i = 0; i < lines.Length; i++)
             {
                 ...
                 if (temp.Contains("请参阅最后一页的信息披露和法律声明"))
                     continue;
                 if (temp.Contains("深度研究") || temp.Contains("点评报告"))
                     continue;
                 if(temp.Contains("在法律许可情况下可能为或争取为本报告提到的公司提供财务顾问服务"))
                     continue;
                 if(temp.Contains("交易本报告提到的上市公司所发行的证券或投资标的"))
                     continue;
                 if (temp.Contains("内履行披露义务") && temp.Contains("客户应全面理解本报告结尾"))
                     continue;
                 if(temp.Contains("处的\"法律声明\"。"))
                 continue;
         ...
         public override bool extrcactContent()
         {
             ...
             string regexDate4 = "[0-9]{4}[.][0-9]{2}[.][0-9]{2}"; //日期正则表达式
             Regex regDate4 = new Regex(regexDate4);
             string regexDate5 = "报告日期[ ]*[0-9]{4}[-][0-9]{2}[-][0-9]{2}"; //日期正则表达式
             Regex regDate5 = new Regex(regexDate5);
            ...
            // 嘉欣丝绸  (002404)
             ...
             foreach (string line in lines)//2007年11月21日 杜冬松 010-59734988 duds@pasc.com.cn 
             {
                 ...
                 if (!DateFlag && regDate5.IsMatch(line.Trim()))
                 {
                     Regex reg1 = new Regex("[0-9]{4}[-][0-9]{2}[-][0-9]{2}");
                 ...
                 if (!DateFlag && regDate4.IsMatch(line.Trim()))
                 {
                     Regex reg1 = new Regex("[0-9]{4}[.][0-9]{2}[.][0-9]{2}");
                     var mat = reg1.Match(line.Trim());
                     Date = mat.ToString();
                 ...
                 if (!StockFlag && regStock.IsMatch(line.Trim()))
                 {
                     ...
                     Regex reg1 = new Regex("[\u4E00-\u9FA5]{4}");
                     var mat = reg1.Match(stockstr);
                     Regex reg2 = new Regex("[\u4E00-\u9FA5]{3}");
                     var mat2 = reg2.Match(stockstr);
                     if (mat.ToString().Equals(""))
                     {
                         StockName = mat2.ToString();
                     }
                     StockName = mat.ToString();

                     Regex reg3 = new Regex("[0-9]{6}");
                     var mat3 = reg3.Match(stockstr);
                     StockCode = mat3.ToString();
                     StockFlag = true;
                 }
