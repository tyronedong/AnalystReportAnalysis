...
{
    //东北证券的抽取方法
    public class DongBeiStock : StockData
    {
        List<string> subjects;
        public DongBeiStock() { }
        public DongBeiStock(StockData stockData) //高于人名的号码不可以要，抽出来的高于人名的号码可能是假的
        {
        ...
        public string extractContent(string[] lines)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (temp.Contains("请务必阅读正文后的声明及说明"))
                    continue;
                if (temp.Contains("公司点评报告"))
                    continue;
                if (temp.Contains("公司研究报告"))
                    continue;
                if (temp.Contains("本报告中的信息均来源于公开数据"))
                    continue;
                if (temp.Contains("我公司及其雇员对使用本报告及其内容所引发的任何直接或间接损失概不负责"))
                    continue;
                 if (temp.Contains("还可能为这些公司提供或争取提供投资银行业务服务"))
                    continue;
